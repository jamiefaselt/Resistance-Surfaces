...
cattle.sales<- read.csv("/Users/jamiefaselt/Resistance-Surfaces/Data/cattle_sales_dollar.csv")
cattle.sales$Value <- gsub(",","",cattle.sales$Value)
cattle.sales$Value <- as.numeric(cattle.sales$Value)

# join
cattle.spatial <- left_join(counties, cattle.sales) 
plot(cattle.spatial)
# double check projection
st_crs(counties) == st_crs(cattle.spatial) #true

#subset to relevant variables
#cattle.sales.sub <- cattle.spatial %>% 
...
st_write(cattle.spatial,"cattle_sales_head.shp", overwrite=TRUE)
writeRaster(rstr, "cattle_sales_head.tif", overwrite=TRUE)
